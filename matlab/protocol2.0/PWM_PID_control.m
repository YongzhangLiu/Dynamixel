...
while 1
    ...
    while 1
        ...
        % PID Block
        dt = toc(t);  % duration of current loop
        if dt == 0
            dt = eps;  % MATLAB's smallest float > 0
        end
        t = tic;
        error_integral = error_integral + dxl_error * dt;
        error_derivative = (dxl_error - dxl_error_prev) / dt;
        
        dxl_goal_pwm = Kp * dxl_error + Ki * error_integral + Kd * error_derivative;
